...
export class NaverShoppingRankChecker {
  ...
  async searchAllProductsAndSave(
    ...
    try {
      const result = await this.findProductRank(searchQuery, targetProductName, targetMallName, targetBrand, maxPages)
      ...
      if (result.found && result.data) {
        ...
        return { success: false, error: '검색 결과에서 상품을 찾을 수 없습니다.' }
      }
    } catch (error) {
