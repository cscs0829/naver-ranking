                                        ...
                                        {execution.results.length === 0 ? (
                                          ...
                                          execution.results
                                            ...
                                            <div 
                                              ...
                                              onClick={() => {
                                                // 네이버 쇼핑의 올바른 페이지 파라미터 사용
                                                const page = Math.floor((result.total_rank - 1) / 40) + 1;
                                                const link = `https://search.shopping.naver.com/search/all?query=${encodeURIComponent(selectedSchedule.search_query)}&pagingIndex=${page}&pagingSize=40&productSet=total&sort=rel&viewType=list`;
                                            ...
                                            >
                                              ...
                                              <div className="hidden sm:flex items-center justify-between">
                                                ...
                                                <a href={`https://search.shopping.naver.com/search/all?query=${encodeURIComponent(selectedSchedule.search_query)}&pagingIndex=${Math.floor((result.total_rank - 1) / 40) + 1}&pagingSize=40&productSet=total&sort=rel&viewType=list`} target="_blank" rel="noopener noreferrer" className="hidden sm:flex items-center gap-1 px-3 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors text-sm"><ExternalLink className="w-4 h-4" />바로가기</a>
                                              </div>
                                            </div>
                                          ))
                                        )}
