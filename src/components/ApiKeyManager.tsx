  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-slate-600 dark:text-slate-400 text-lg">네이버 DataLab API 키(검색어 트렌드/쇼핑인사이트)를 안전하게 관리하세요</p>
      </div>
      ...
      <div className="flex justify-center">
        <motion.button
          ...
          onClick={() => {
            ...
            setProfileForm({ name: '', clientId: '', clientSecret: '', apiType: 'insights', makeDefault: false })
            setError('')
            setSuccess('')
          }}
            ...
            <form onSubmit={handleSaveProfile} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="insights">DataLab 검색어 트렌드/인사이트</option>
                  </select>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex items-center space-x-3">
                ...
                <label htmlFor="makeDefault" className="text-sm text-slate-700 dark:text-slate-300">
                  이 프로필을 {profileForm.apiType === 'shopping' ? '쇼핑 검색 API' : 'DataLab(검색어 트렌드/인사이트)'}의 기본으로 설정
                </label>
              </div>
                  ...
                  <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        ...
                        {p.api_type === 'shopping' ? '쇼핑 검색' : 'DataLab(트렌드/인사이트)'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-2">
                        {!p.is_default && p.is_active && (
                          ...
                          >
                            {p.api_type === 'shopping' ? '쇼핑검색 기본' : 'DataLab 기본'}
                          </motion.button>
                        )}
                        <motion.button 
                          whileHover={{ scale: 1.05 }}
